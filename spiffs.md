...
```
{
    "spiffs":
    {
        "rewrite":true,     //флаг перезаписи данных, если отсутствует, то данные добавлены к файлу
...
```
{
    "spiffs":
    {
        "gc": 128000 //сколько освободить байт (CONFIG_SPIFFS_GC_MAX_RUNS should be set at least to the maximum expected size_to_gc, divided by 4096)
    }
}
```
Ответ
```
{
    "spiffs":
    {
        "total":4751681,//всего байт
        "used":274845//занято байт
    }
}
```
## Транзакция записи файла.
Для атомарной записи данных в файлы и/или удаления файлов осуществляются следующие действия:
1. Отменяется предыдущая незавершенная транзакция.
2. К имени записываемых файлов добавляется '$' и они последовательно записываются в устройство.
3. К имени удаляемых файлов добавляется '!' и создаются пустые файлы.
4. Завершается транзакция (при необходимости передаётся список директорий на очистку старых файлов).
