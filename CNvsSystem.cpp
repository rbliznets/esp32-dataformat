/*!
	\file
	\brief Класс для работы с NVS.
	\authors Близнец Р.А. (r.bliznets@gmail.com)
	\version 1.0.0.0
	\date 02.05.2024
*/

#include "CNvsSystem.h"
#include "esp_log.h"
#include <cstdio>
#include "nvs_flash.h"
#include "nvs.h"
#include "esp_system.h"

static const char *TAG = "nvs";

/**
 * @brief Инициализация NVS (Non-Volatile Storage) памяти
 * @return true если инициализация успешна, false в случае ошибки
 * 
 * Функция инициализирует NVS память для основного раздела и дополнительного раздела "nvs2".
 * При необходимости выполняет очистку памяти если обнаружены ошибки инициализации.
 */
bool CNvsSystem::init()
{
	esp_err_t err = nvs_flash_init();
	
	// Проверяем необходимость очистки основного раздела NVS
	if (err == ESP_ERR_NVS_NO_FREE_PAGES || err == ESP_ERR_NVS_NEW_VERSION_FOUND)
	{
		ESP_LOGW(TAG, "nvs_flash_erase (%d)", err);
		ESP_ERROR_CHECK(nvs_flash_erase());
		err = nvs_flash_init();
	}
	
	if (err == ESP_OK)
	{
		// Инициализируем дополнительный раздел NVS "nvs2"
		err = nvs_flash_init_partition("nvs2");
		if (err == ESP_ERR_NVS_NO_FREE_PAGES || err == ESP_ERR_NVS_NEW_VERSION_FOUND)
		{
			ESP_LOGW(TAG, "nvs_flash_erase (%d)", err);
			ESP_ERROR_CHECK(nvs_flash_erase_partition("nvs2"));
			err = nvs_flash_init_partition("nvs2");
		}
	}
	else
	{
		ESP_LOGE(TAG, "nvs failed (%d)", err);
	}
	
	return (err == ESP_OK);
}

/**
 * @brief Деинициализация NVS памяти
 * 
 * Функция освобождает ресурсы NVS памяти перед завершением работы.
 */
void CNvsSystem::free()
{
	nvs_flash_deinit();
}

/**
 * @brief Обработка команд работы с NVS памятью
 * @param cmd JSON-команда с параметрами работы с NVS
 * @param answer JSON-ответ с результатами выполнения команд
 * 
 * Поддерживаемые команды:
 * - "clear": очистка NVS памяти
 * - "reset": перезагрузка устройства
 * - Работа с переменными различных типов (u8, i8, i16, i32, u32, float, double)
 */
void CNvsSystem::command(json& cmd, json& answer)
{
	if(cmd.contains("nvs"))
	{
		nvs_handle_t nvs_handle;
		error_t err;
		
		// Команда очистки NVS памяти
		if (cmd["nvs"].contains("clear"))
		{
			err = nvs_flash_deinit();
			err |= nvs_flash_erase();
			err |= nvs_flash_init();
			answer["nvs"] = err;
		}
		// Команда перезагрузки устройства
		else if (cmd["nvs"].contains("reset"))
		{
			esp_restart();
		}
		// Работа с переменными в NVS
		else if (nvs_open("nvs", NVS_READWRITE, &nvs_handle) == ESP_OK)
		{
			// Проверяем наличие имени переменной
			if (cmd["nvs"].contains("name") && cmd["nvs"]["name"].is_string())
			{
				std::string name = cmd["nvs"]["name"].template get<std::string>();
				answer["nvs"]["name"] = name;
				
				// Определяем тип переменной (по умолчанию u16)
				std::string tp = "u16";
				if (cmd["nvs"].contains("type") && cmd["nvs"]["type"].is_string())
				{
					tp = cmd["nvs"]["type"].template get<std::string>();
				}
				
				// Работа с переменной типа uint8_t
				if (tp == "u8")
				{
					uint8_t u8;
					// Запись значения
					if (cmd["nvs"].contains("value") && cmd["nvs"]["value"].is_number_unsigned())
					{
						u8 = cmd["nvs"]["value"].template get<uint8_t>();
						if ((err = nvs_set_u8(nvs_handle, name.c_str(), u8)) == ESP_OK)
						{
							answer["nvs"]["value"] = u8;
							err = nvs_commit(nvs_handle); // Подтверждаем запись
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
					// Чтение значения
					else
					{
						if ((err = nvs_get_u8(nvs_handle, name.c_str(), &u8)) == ESP_OK)
						{
							answer["nvs"]["value"] = u8;
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
				}
				// Работа с переменной типа int8_t
				else if (tp == "i8")
				{
					int8_t i8;
					// Запись значения
					if (cmd["nvs"].contains("value") && cmd["nvs"]["value"].is_number_integer())
					{
						i8 = cmd["nvs"]["value"].template get<int8_t>();
						if ((err = nvs_set_i8(nvs_handle, name.c_str(), i8)) == ESP_OK)
						{
							answer["nvs"]["value"] = i8;
							err = nvs_commit(nvs_handle);
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
					// Чтение значения
					else
					{
						if ((err = nvs_get_i8(nvs_handle, name.c_str(), &i8)) == ESP_OK)
						{
							answer["nvs"]["value"] = i8;
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
				}
				// Работа с переменной типа int16_t
				else if (tp == "i16")
				{
					int16_t i16;
					// Запись значения
					if (cmd["nvs"].contains("value") && cmd["nvs"]["value"].is_number_integer())
					{
						i16 = cmd["nvs"]["value"].template get<int16_t>();
						if ((err = nvs_set_i16(nvs_handle, name.c_str(), i16)) == ESP_OK)
						{
							answer["nvs"]["value"] = i16;
							err = nvs_commit(nvs_handle);
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
					// Чтение значения
					else
					{
						if ((err = nvs_get_i16(nvs_handle, name.c_str(), &i16)) == ESP_OK)
						{
							answer["nvs"]["value"] = i16;
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
				}
				// Работа с переменной типа int32_t
				else if (tp == "i32")
				{
					int32_t i32;
					// Запись значения
					if (cmd["nvs"].contains("value") && cmd["nvs"]["value"].is_number_integer())
					{
						i32 = cmd["nvs"]["value"].template get<int32_t>();
						if ((err = nvs_set_i32(nvs_handle, name.c_str(), i32)) == ESP_OK)
						{
							answer["nvs"]["value"] = i32;
							err = nvs_commit(nvs_handle);
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
					// Чтение значения
					else
					{
						if ((err = nvs_get_i32(nvs_handle, name.c_str(), &i32)) == ESP_OK)
						{
							answer["nvs"]["value"] = i32;
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
				}
				// Работа с переменной типа uint32_t
				else if (tp == "u32")
				{
					uint32_t u32;
					// Запись значения
					if (cmd["nvs"].contains("value") && cmd["nvs"]["value"].is_number_unsigned())
					{
						u32 = cmd["nvs"]["value"].template get<uint32_t>();
						if ((err = nvs_set_u32(nvs_handle, name.c_str(), u32)) == ESP_OK)
						{
							answer["nvs"]["value"] = u32;
							err = nvs_commit(nvs_handle);
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
					// Чтение значения
					else
					{
						if ((err = nvs_get_u32(nvs_handle, name.c_str(), &u32)) == ESP_OK)
						{
							answer["nvs"]["value"] = u32;
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
				}
				// Работа с переменной типа float
				else if (tp == "float")
				{
					float d;
					uint32_t u32;
					// Запись значения
					if (cmd["nvs"].contains("value") && cmd["nvs"]["value"].is_number())
					{
						d = cmd["nvs"]["value"].template get<float>();
						std::memcpy(&u32, &d, sizeof(d)); // Преобразуем float в uint32_t для хранения
						if ((err = nvs_set_u32(nvs_handle, name.c_str(), u32)) == ESP_OK)
						{
							answer["nvs"]["value"] = d;
							err = nvs_commit(nvs_handle);
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
					// Чтение значения
					else
					{
						if ((err = nvs_get_u32(nvs_handle, name.c_str(), &u32)) == ESP_OK)
						{
							std::memcpy(&d, &u32, sizeof(d)); // Преобразуем uint32_t обратно в float
							answer["nvs"]["value"] = d;
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
				}
				// Работа с переменной типа double
				else if (tp == "double")
				{
					double d;
					uint64_t d64;
					// Запись значения
					if (cmd["nvs"].contains("value") && cmd["nvs"]["value"].is_number())
					{
						d = cmd["nvs"]["value"].template get<double>();
						std::memcpy(&d64, &d, sizeof(d)); // Преобразуем double в uint64_t для хранения
						if ((err = nvs_set_u64(nvs_handle, name.c_str(), d64)) == ESP_OK)
						{
							answer["nvs"]["value"] = d;
							err = nvs_commit(nvs_handle);
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
					// Чтение значения
					else
					{
						if ((err = nvs_get_u64(nvs_handle, name.c_str(), &d64)) == ESP_OK)
						{
							std::memcpy(&d, &d64, sizeof(d)); // Преобразуем uint64_t обратно в double
							answer["nvs"]["value"] = d;
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
				}
				// Работа с переменной типа uint16_t (по умолчанию)
				else
				{
					uint16_t u16;
					// Запись значения
					if (cmd["nvs"].contains("value") && cmd["nvs"]["value"].is_number_unsigned())
					{
						u16 = cmd["nvs"]["value"].template get<uint16_t>();
						if ((err = nvs_set_u16(nvs_handle, name.c_str(), u16)) == ESP_OK)
						{
							answer["nvs"]["value"] = u16;
							err = nvs_commit(nvs_handle);
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
					// Чтение значения
					else
					{
						if ((err = nvs_get_u16(nvs_handle, name.c_str(), &u16)) == ESP_OK)
						{
							answer["nvs"]["value"] = u16;
						}
						else
						{
							answer["nvs"]["error"] = err;
						}
					}
				}
				
				// Проверяем наличие ошибок
				if(err != ESP_OK)
				{
					answer["nvs"]["error"] = err;
				}
			}
			// Закрываем дескриптор NVS
			nvs_close(nvs_handle);
		}
	}
}